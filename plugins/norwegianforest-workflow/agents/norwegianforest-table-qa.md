...
skills:
    ...
    - norwegianforest-knowledge-base:norwegianforest-testing-architecture
tools:
    - Read
    - Write
    - Edit
...
開始撰寫前，載入 `norwegianforest-knowledge-base:norwegianforest-testing-architecture`，再依腳本類型參考 `references/03-script-type-testing.md` 的對應模板。
...
```
載入 norwegianforest-knowledge-base:norwegianforest-testing-architecture
...
若腳本涉及 TodoJob：
```
載入 norwegianforest-engineering-mindset:javacat-todojob-mechanism
```

若腳本邏輯複雜（hook 觸發、ignorePolicy 等）：
```
載入 norwegianforest-knowledge-base:function-script-context
```
