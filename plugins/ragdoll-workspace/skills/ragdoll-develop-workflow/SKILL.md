...
```mermaid
flowchart TD
    A([使用者提出規格需求]) --> A1["詢問使用者：<br>是否需要 brainstorming？"]
    A1 -- 是 --> B["superpowers:brainstorming<br>蒐集需求與工具"]
...
當使用者提出規格開發需求時，**MUST** 先詢問使用者：

> 「需求已收到。請選擇下一步：
> 1. Brainstorming — 先討論實作細節與不確定點，再進入 Plan 撰寫
> 2. 直接寫 Plan — 跳過 brainstorming，直接進入 Plan 撰寫」

- **若使用者選擇 1**：呼叫 `superpowers:brainstorming` 蒐集需求與上下文，完成後進入 Step 2。
- **若使用者選擇 2**：直接進入 Step 2 擬定 Plan。
