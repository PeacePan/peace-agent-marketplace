...
- **讀者**：任何關係人（客服、營運、店長、PM……），**不是 QA 工程師專屬**——寫的時候假設對方完全沒寫過程式
- **格式**：ADF `taskList` / `taskItem`（checkbox 可勾選）
- **內容**：每一條是「（操作產品本身既有的介面或實體裝置）→ 肉眼可觀察的具體結果」，驗收者不用問任何人、照著做就能自己判斷過不過
- **鐵律**：這一欄**不得包含任何工程類驗收動作**——不能要求驗收者打開終端機（`curl`、`npm`、`git`）、登入雲端主控台（AWS/GCP Console）、進資料庫或內部監控面板（Sentry、CloudWatch）、跑測試腳本、改設定重新部署，或「手動觸發某個內部函式／API」。唯一例外：那個操作本身就是產品給終端使用者的正常功能（例如門市 POS 系統匣選單裡的「立即上傳」按鈕，門市人員平常就會用，不是工程工具）
- **禁止**：狀態描述（「服務跑起來」、「健康檢查通過」是錯的，寫了等於沒寫可操作步驟）

**如果這張票的改動完全沒有終端使用者可觸及的介面**（純後端 / 純 infra，例如單純一次部署、一個內部批次）：不要硬套 taskList 湊出工程步驟。往上一層回推「這個技術動作最終服務了哪個下游、下游會出現什麼看得見的行為」，改用那個下游行為當驗收條件；若下游行為要等好幾張票之後才會出現，這類驗證本來就屬於工程團隊自己的職責，移到「討論紀錄」給工程師自行核對，「驗收條件」欄位改寫成由工程師回報、PM/PO 認可的高層描述（例如「工程團隊確認服務已上線且運作正常，可截圖或連結佐證」）。

**重現困難情境的技巧：找物理／操作等價物**。很多「工程師才知道怎麼構造」的情境，其實在現實世界找得到對應的實體操作，優先用它取代程式碼層級的模擬：

| 工程情境 | 物理／操作等價 |
|---|---|
| 裝置連線失敗、API 呼叫逾時 | 直接拔掉裝置連接線／關閉裝置電源／關閉網路 |
| 背景排程搶在資料寫入前先執行 | 用產品既有的「立即執行／立即上傳」之類按鈕手動觸發，而非乾等排程 |
| 外部服務短暫異常 | 若產品本身有模擬異常的正式功能（如「離線模式」開關）優先用它 |

只有真的找不到任何等價操作、且這個改動完全沒有終端使用者介面時，才退回工程師自行驗證，並把它移出「驗收條件」欄位。

**範例（好）**——情境：修正「印表機斷線時可能漏開發票」的 bug：
- ☐ 把發票機的連接線拔掉或關閉電源，然後在收銀畫面完成一筆結帳。確認：結帳照常完成、不會因為印表機斷線而卡住整筆交易
- ☐ 保持印表機斷線超過 10 分鐘，確認畫面或系統匣跳出提醒「這筆交易需要人工確認印表機狀態」的通知
- ☐ 重新接上印表機、完成一筆全新結帳，確認印出的紙本發票號碼跟後臺查詢畫面上顯示的號碼完全一致

**範例（壞）**：
- ☐ 在終端機執行 `curl https://pos-proxy-development.wonderpet.asia/health`，看到 HTTP 200 ← 要求驗收者會用終端機
- ☐ 在 AWS console 確認 ECS service 的 task 狀態為 RUNNING ← 要求驗收者有雲端主控台權限與知識
- ☐ 暫時把 secret 設成空值並重新部署，確認服務啟動失敗 ← 要求驗收者會改設定、會重新部署
- ☐ van 健康檢查通過 ← 沒有具體操作步驟，也是工程用語
...
| 驗收條件寫「程式碼合併到 main」 | 那是工程動作，不是任何關係人可驗收的結果 | 改寫成上線後從產品端能看到的效果（例如「門市在 X 畫面操作 Y，得到 Z 結果」）；若這個階段真的完全沒有終端使用者可驗證的效果，代表這條本來就不屬於「驗收條件」，應移到「討論紀錄」作為工程師自己的部署檢查清單 |
| 驗收條件寫「用 curl / AWS Console 確認服務正常」 | 預設驗收者會用工程工具，等於只有工程師能驗收 | 找物理／操作等價物（拔裝置線、用產品既有按鈕手動觸發等）；真的找不到就承認這是工程師職責，移出驗收條件欄位 |
